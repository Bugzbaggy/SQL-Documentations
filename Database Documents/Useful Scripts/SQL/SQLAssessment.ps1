SqlAssessment_CL
//| extend LatestAssessment = TimeGenerated
| extend DefaultRule = RawData
| parse DefaultRule with * "DefaultRuleset," DefaultRule
| summarize LatestAssessment = max(TimeGenerated) by DefaultRule, _ResourceId, Computer, RawData
| where RawData contains "SystemHealth"
//| distinct Computer, LatestAssessment, RawData, DefaultRule, _ResourceId
| project Computer, LatestAssessment, RawData, DefaultRule, _ResourceId
...
| summarize LatestAssessment = max(TimeGenerated) by SQLServer, SID, Environment, TargetType, TargetName, Severity, Tags, CheckId, Sev, Message   
| where Sev >= 0 
...
| order by SeverityCode desc, Issues desc
