...
$SQLVersion = Invoke-Sqlcmd -Query "SELECT SUBSTRING(@@VERSION,0,54);" -ServerInstance $InstanceName
...
IsPrimaryServer DESC;" -ServerInstance "$InstanceName" 
    ...
    Foreach($AGstate in $AGstates){
     ...
     If($AGstate.IsPrimaryServer -eq 1){
        ...
        MODIFY REPLICA ON N'$($PrimaryInstance)' WITH (FAILOVER_MODE = AUTOMATIC)
        GO
        USE [master]
        GO
        ALTER AVAILABILITY GROUP [$($AGstate.AGname)]
        MODIFY REPLICA ON N'$($SecondaryInstance)' WITH (FAILOVER_MODE = AUTOMATIC)
        GO";
