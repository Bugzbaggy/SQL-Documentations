<#
    ...
    .DESCRIPTION 
        This step will initiate failover
...
IsPrimaryServer DESC;" -ServerInstance "$InstanceName" 
    ...
    Foreach($AGstate in $AGstates){
      ...
      If($AGNode -eq $SecondaryInstance.Split('\')[0]){
        ...
        $AGPath = “SQLSERVER:\Sql\$($SecondaryInstance)\AvailabilityGroups\$($AGstate.AGname)”
		Switch-SqlAvailabilityGroup -Path $AGPath -Force
        }
	...
	Write-Host "
		##########################################
		Failover succeeded...
		Kindly validate result below" -ForegroundColor Green
