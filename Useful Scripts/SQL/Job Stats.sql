...
ISNULL(SP.[name], 'Unknown') AS JobOwner, ISNULL(JS.[enabled], 0) AS ScheduleIsEnabled
...
LEFT JOIN [msdb].[dbo].[sysjobsteps] AS SJS ON j.job_id=sjs.job_id
...
--WHERE j.name like '%blitz%'
WHERE J.[name] like '%F6A%HANA%'
AND J.enabled = '1'
--AND JS.[enabled] = '1'
ORDER BY
J.[name] ASC
